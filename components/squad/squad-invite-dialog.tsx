...
import { AlertCircle, Check, Loader2, Share2, UserPlus } from "lucide-react";
...
/** Same shape as invite-friends-dialog.tsx (pick Fit Circle members, notify
 * them) but targeting invite_to_squad — any current squad member can invite
 * more circle-mates while the lobby's still open — plus the join code itself
 * for a quick share-sheet invite that doesn't need picking names at all. */
export function SquadInviteDialog({ trigger, sessionId, joinCode }: { trigger: React.ReactNode; sessionId: string; joinCode: string }) {
  ...
  const shareUrl = typeof window !== "undefined" ? `${window.location.origin}/squad/join?code=${joinCode}` : "";
  ...
  async function sendInvites() {
    setError(null);
    const supabase = createClient();
    const { error } = await supabase.rpc("invite_to_squad", { p_session_id: sessionId, p_profile_ids: [...selected] });
    if (error) {
  ...
  async function share() {
    if (navigator.share) {
      await navigator.share({ title: "Join my squad session", text: `Join code: ${joinCode}`, url: shareUrl }).catch(() => {});
    } else {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Join code: <span className="font-mono font-semibold text-foreground">{joinCode}</span></DialogDescription>
        </DialogHeader>

        <Button variant="outline" className="w-full" onClick={share}>
          <Share2 /> Share join link
        </Button>

        {loading ? (
          <p className="py-4 text-center text-sm text-muted-foreground">Loading your Fit Circle…</p>
        ) : friends.length === 0 ? (
          <EmptyState icon={UserPlus} title="No Fit Circle friends yet" description="Share the join code above instead." />
        ) : (
        ...
        {friends.length > 0 && (
          <Button className="mt-2 w-full" disabled={selected.size === 0} onClick={sendInvites}>
